...
* 1.audio 
	...
	* 属性:
		...
		* controls:控制条
		* muted  :静音
		* preload:预加载(当使用autoplay的时候,preload自动失效)
* 2.Video
	* 加载视频;IE9以下的版本不支持
	* 支持的格式:.mp4/.ogg/.webm 
	* 属性:
...
* 4.canvas
	* 画布。是一个容器元素。
	* 注意:
		* 1.单独使用canvas没有什么意义,它必须结合Javascript使用;它的功能提现是通过Javascript提现出来的。 
...
* 5. required 
	* 约束表单元素在提交时必须输入值;用于表单组件中,需要结合提交按钮使用 
* 6. pattern:
...
### 10.input属性
* 1.autocomplete:自动完成
	* 用来帮助用户输入,每一次输入的内容,浏览器是否保存输入的值,以备讲来使用。值有on/off,默认为on
	* 为了保护敏感数据(如:用户账号,密码等),避免本地浏览器对它们 
* 2.autofocus:自动获焦
* 3.step:步长
* 4.multiple:多选
* 5.pattern:正则匹配
